...
public partial class DriveItemViewModel : ObservableObject
{
    ...
    public DriveItemViewModel(object connection, IMountManager mountManager, ISftpService sftpService, IFtpService ftpService)
    {
    ...
    private async Task MountAsync()
    {
        ...
        try
        {
            ...
            {
                await _mountManager.MountAsync(sftp, DriveLetter);
            }
            else if (Connection is FtpConnection ftp)
            {
                await _mountManager.MountAsync(ftp, DriveLetter);
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task TestConnectionAsync()
    {
        ...
        try
        {
            if (Connection is SftpConnection sftp)
            {
                var (success, message) = await _sftpService.TestConnectionAsync(sftp);
                if (!success)
                {
            ...
            {
                var (success, message) = await _ftpService.TestConnectionAsync(ftp);
                if (!success)
                {
                    ErrorMessage = message;
                }
