...
namespace RcloneGui.Features.Ftp.ViewModels;
...
public partial class AddFtpConnectionViewModel : ObservableObject
{
    ...
    public AddFtpConnectionViewModel(IFtpService ftpService, IConfigManager configManager)
    {
    ...
    private void LoadAvailableDriveLetters()
    {
        ...
        foreach (var letter in _ftpService.GetAvailableDriveLetters())
        {
    ...
    private async Task TestConnectionAsync()
    {
        ...
        try
        {
            ...
            {
                testConnection.ObscuredPassword = await _ftpService.ObscurePasswordAsync(Password);
            }

            var (success, message) = await _ftpService.TestConnectionAsync(testConnection);
            TestSuccess = success;
            TestResult = message;
    ...
    public async Task<bool> SaveAsync()
    {
        ...
        try
        {
            ...
            if (!IsAnonymous && !string.IsNullOrEmpty(Password))
            {
                connection.ObscuredPassword = await _ftpService.ObscurePasswordAsync(Password);
            }

            // Create rclone remote
            var created = await _ftpService.CreateRemoteAsync(connection);
            if (!created)
            {
