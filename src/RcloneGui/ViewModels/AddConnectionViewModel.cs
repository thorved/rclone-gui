...
public partial class AddConnectionViewModel : ObservableObject
{
    ...
    public AddConnectionViewModel(IRcloneService rcloneService, IConfigManager configManager)
    {
    ...
    private async Task TestConnectionAsync()
    {
        ...
        try
        {
            ...
            {
                testConnection.ObscuredKeyPassphrase = await _rcloneService.ObscurePasswordAsync(KeyPassphrase);
            }

            var (success, message) = await _rcloneService.TestConnectionAsync(testConnection);
            TestSuccess = success;
            TestResult = message;
    ...
    public async Task<bool> SaveAsync()
    {
        ...
        try
        {
            ...
            {
                connection.ObscuredKeyPassphrase = await _rcloneService.ObscurePasswordAsync(KeyPassphrase);
            }

            // Create rclone remote
            var created = await _rcloneService.CreateSftpRemoteAsync(connection);
            if (!created)
            {
