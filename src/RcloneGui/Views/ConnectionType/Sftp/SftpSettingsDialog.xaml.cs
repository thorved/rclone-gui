...
public sealed partial class SftpSettingsDialog : ContentDialog
{
    ...
    public SftpSettingsDialog()
    {
        ...
        _rcloneService = App.Services.GetRequiredService<IRcloneService>();
    ...
    private async Task CreateConnectionAsync()
    {
        ...
        {
            obscuredPassword = await _rcloneService.ObscurePasswordAsync(PasswordBox.Password);
        }

        string? obscuredKeyPassphrase = null;
        if (!isPasswordAuth && !string.IsNullOrEmpty(KeyPassphraseBox.Password))
        {
            obscuredKeyPassphrase = await _rcloneService.ObscurePasswordAsync(KeyPassphraseBox.Password);
        }
        ...
        await _rcloneService.CreateSftpRemoteAsync(connection);
    ...
    private async Task UpdateConnectionAsync()
    {
        ...
        if (isPasswordAuth && !string.IsNullOrEmpty(PasswordBox.Password))
        {
            _connection.ObscuredPassword = await _rcloneService.ObscurePasswordAsync(PasswordBox.Password);
        }
        ...
        if (!isPasswordAuth && !string.IsNullOrEmpty(KeyPassphraseBox.Password))
        {
            _connection.ObscuredKeyPassphrase = await _rcloneService.ObscurePasswordAsync(KeyPassphraseBox.Password);
        }
        ...
        await _rcloneService.CreateSftpRemoteAsync(_connection);
